...
const PORT = 8993;
const VIDEO_PATH = path.join(__dirname, 'public', 'videos', 'gemini-user-video-3.mp4');
...
const server = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    req.on('end', () => {
      ...
      fs.writeFileSync(path.join(OUT_DIR, `v3-play-frame-${idx}.jpg`), Buffer.from(base64Data, 'base64'));
      console.log(`Saved v3-play-frame-${idx}.jpg at video currentTime ${ct}s`);
      res.writeHead(200);
      res.end('ok');
    });
