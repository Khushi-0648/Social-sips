...
const PORT = 8999;
const VIDEO_IN = path.join(__dirname, 'public', 'videos', 'gemini-user-video-2.mp4');
const WEBM_OUT = path.join(__dirname, 'public', 'videos', 'gemini-user-video-2.webm');
const POSTER_OUT = path.join(__dirname, 'public', 'images', 'story', 'gemini-video-2-poster.jpg');

const html = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <title>Render Video 2 HD</title>
...
<body>
  ...
  <script>
    ...
    async function startProcessing() {
      // First, capture a poster frame at 0.8s (clean milk pour)
      v.currentTime = 0.8;
      ...
      recorder.onstop = async () => {
        console.log('Recorder stopped. Sending ' + chunks.length + ' chunks...');
...
server.listen(PORT, () => {
  console.log('Encoding server listening on port', PORT);
});
