...
const PORT = 8993;
const VIDEO_OUTPUT = path.join(__dirname, 'public', 'videos', 'custom-hero-coffee.webm');
const POSTER_OUTPUT = path.join(__dirname, 'public', 'images', 'story', 'custom-hero-poster.jpg');
...
<head>
  <meta charset="utf-8">
  <title>Render Custom Hero Video</title>
  <style>
...
<body>
  ...
  <script>
    ...
    function checkLoaded() {
      loadedCount++;
      if (loadedCount === 3) {
        console.log('All 3 frames loaded, starting render...');
        startRecording();
      }
    }
    ...
    const LOOP_DURATION = 12.0; // 12-second slow-motion loop

    function getFrameBlend(t) {
      const p = (t % LOOP_DURATION);
      // Timeline:
      // 0.0 - 3.5: Frame 1 (Crown splash & milk stream)
      // 3.5 - 5.0: Cross-fade Frame 1 -> Frame 2 (Ripples spreading)
      // 5.0 - 7.5: Frame 2 (Expanding ripple waves)
      // 7.5 - 9.0: Cross-fade Frame 2 -> Frame 3 (Latte art heart blossoming)
      // 9.0 - 10.8: Frame 3 (Latte art steaming)
      // 10.8 - 12.0: Cross-fade Frame 3 -> Frame 1
    ...
    function render(timestamp) {
      ...
      // 1. Ken Burns slow-motion camera drift (smooth 12s sine cycle)
      ...
      // 2. Base & Cross-fade image rendering
      ...
      // 3. Fluid Surface Ripples (cup center: x=712, y=490)
      // Add delicate animated ripple sheen in the coffee cup
      ctx.save();
      ...
      for (let r = 0; r < 4; r++) {
        ...
        const radiusY = radiusX * 0.38; // perspective foreshortening
      ...
      // 4. Milk Stream Specular Flow Animation (stream x ~ 712, y: 0 to 420)
      // Only draw during pour phases (when not purely frame 3)
      ...
      // 6. Sunlight Glow Shimmer & Light Rays (Top-Left Sunbeam)
      ctx.save();
      ctx.globalCompositeOperation = 'screen';
    ...
    async function startRecording() {
      // 1. Render single crisp poster frame at t=0.5s and upload
      render(500);
      const posterData = canvas.toDataURL('image/jpeg', 0.96);
      await fetch('/save-poster', { method: 'POST', body: posterData });
      console.log('Poster frame saved!');

      // 2. Start 12-second 30fps canvas stream recording
      ...
      const recorder = new MediaRecorder(stream, {
        mimeType: mimeType,
        videoBitsPerSecond: 14000000 // 14 Mbps ultra-clear
      });
      ...
      recorder.onstop = async () => {
        console.log('Recording stopped, assembling chunks...');
      ...
      // Record for 12.2 seconds (full loop)
      setTimeout(() => {
        recorder.stop();
...
const server = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    res.end(html);
  } else if (req.url === '/save-poster' && req.method === 'POST') {
    let body = '';
    req.on('data', c => body += c);
    req.on('end', () => {
      const base64Data = body.replace(/^data:image\/jpeg;base64,/, '');
      fs.writeFileSync(POSTER_OUTPUT, Buffer.from(base64Data, 'base64'));
      console.log('Saved poster to:', POSTER_OUTPUT);
      res.writeHead(200);
      res.end('ok');
    });
    ...
    req.on('end', () => {
      ...
      console.log('Saved custom hero video to:', VIDEO_OUTPUT, '(' + (buffer.length / 1024 / 1024).toFixed(2) + ' MB)');
      res.writeHead(200);
      res.end('ok');
      setTimeout(() => {
        server.close();
...
server.listen(PORT, async () => {
  ...
  console.log('Spawning Chrome to render custom hero video...');

  const chrome = spawn('C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', [
    '--headless=new',
    '--disable-gpu',
    '--user-data-dir=C:\\Users\\satya\\.gemini\\antigravity\\scratch\\cdp-render-hero-vid',
